...
{
    /// <summary>
    /// MapleStory's implementation of CRC32
    /// </summary>
    public static class CRC32
    {
        private static readonly UInt32[] Crc32Table = new UInt32[0x100] {
        ...
        public static UInt32 ComputeChecksum(byte[] data)
        {
            UInt32 result = 0;
            for (int i = 0; i < data.Length; i++)
                result = Crc32Table[data[i] ^ (result >> 24)] ^ (result << 8);
            return result;
        }

        public unsafe static UInt32 ComputeChecksum(uint data)
        {
            UInt32 result = 0;
            ...
            for (int i = 0; i < 4; i++)
                result = Crc32Table[pbData[i] ^ (result >> 24)] ^ (result << 8);
            return result;
        }
    }
}
